...
def handle_signup():
    ...
    # Strict Validation: Password complexity (at least 8 chars, 1 uppercase, 1 lowercase, 1 digit, 1 special char)
    if not password or len(password) < 8 or \
       not any(c.isupper() for c in password) or \
       not any(c.islower() for c in password) or \
       not any(c.isdigit() for c in password) or \
       not any(not c.isalnum() for c in password):
        return "❌ Registration Failed: Password must be at least 8 characters long, and contain at least one uppercase letter, one lowercase letter, one digit, and one special character.", 400
